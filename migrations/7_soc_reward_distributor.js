const { socPools, INITIAL_SOC_FOR_POOLS } = require('./pools');
...
module.exports = async (deployer, network, accounts) => {
  ...
  const initialCashAmount = unit.muln(INITIAL_SOC_FOR_POOLS).toString();

  const cash = await Cash.deployed();
  const pools = socPools.map(({contractName}) => artifacts.require(contractName));
  ...
  for await (const poolInfo of pools) {
    ...
    console.log('setRewardDistribution pool = ' + pool.address)
    await pool.setRewardDistribution(distributor.address);
  }

  await cash.mint(distributor.address, initialCashAmount);
  console.log(`Deposited ${INITIAL_SOC_FOR_POOLS} BAC to InitialCashDistributor.`);
